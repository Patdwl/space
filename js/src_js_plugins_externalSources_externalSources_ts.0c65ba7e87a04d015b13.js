...
var n2yoFormSubmit = function () {
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#loading-screen').fadeIn(1000, function () {
        var satnum = parseInt(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ext-n2yo').val());
        searchN2yo(satnum);
...
var searchN2yo = function (satNum, analsat) {
    ...
    request.open('GET', "php/get_data.php?type=n&sat=".concat(satNum), true);
...
var searchN2yoOnLoad = function (request, analsat) {
    ...
    if (request.status >= 200 && request.status < 400) {
        // Success!
        var tles = request.response.split('<div id="tle">')[1].split('<pre>')[1].split('\n');
        var TLE1 = tles[1];
        var TLE2 = tles[2];
        ...
        var sat = satSet.insertNewAnalystSatellite(TLE1, TLE2, analsat);
...
var bottomMenuClick = function (iconName) {
    ...
    if (iconName === 'menu-external') {
        if (isExternalMenuOpen) {
            ...
            _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.watchlist.updateWatchlist();
...
var searchCelestrackOnLoad = function (request, analsat) {
    var _a = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs, satSet = _a.satSet, uiManager = _a.uiManager;
    if (request.status >= 200 && request.status < 400) {
        // Success!
        var tles = JSON.parse(request.response).split('\n');
        var TLE1 = tles[1];
        var TLE2 = tles[2];
        if (TLE1.substr(0, 2) !== '1 ')
            throw new Error("N2YO TLE 1 is not a valid TLE -- ".concat(TLE1.substr(0, 2)));
        if (TLE2.substr(0, 2) !== '2 ')
            throw new Error("N2YO TLE 2 is not a valid TLE -- ".concat(TLE2.substr(0, 2)));
        var sat = satSet.insertNewAnalystSatellite(TLE1, TLE2, analsat);
        uiManager.doSearch(sat.SCC_NUM.toString());
    }
    else {
...
var searchCelestrak = function (satNum, analsat) {
    ...
    var request = new XMLHttpRequest();
    request.open('GET', "php/get_data.php?type=c&sat=".concat(satNum), true);
...
var celestrakFormSubmit = function () {
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#loading-screen').fadeIn(1000, function () {
        var satnum = parseInt(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ext-celestrak').val());
...
//# sourceMappingURL=src_js_plugins_externalSources_externalSources_ts.0c65ba7e87a04d015b13.js.map