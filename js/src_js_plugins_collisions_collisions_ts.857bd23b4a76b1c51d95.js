...
var socratesOnSatCruncher = null;
_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.socrates = {
    socratesObjOne: [],
...
var uiManagerInit = function () {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_2___default()('#socrates-menu').on('click', '.socrates-object', function (evt) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_2___default()('#socrates-menu').resizable({
        handles: 'e',
        stop: function () {
...
var adviceReady = function () {
    var aM = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.adviceManager;
    ...
    aM.adviceList.socrates = function () {
        ...
        aM.showAdvice('SOCRATES Near Conjunction List', 'Did you know that objects frequently come close to colliding? Using data from Center for Space Standards and Innovation you can find upcoming possible collisions.', jquery__WEBPACK_IMPORTED_MODULE_2___default()('#menu-satellite-collision'), 'bottom');
    };
...
var bottomMenuClick = function (iconName) {
    if (iconName === 'menu-satellite-collision') {
        if (isSocratesMenuOpen) {
            ...
            _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.hideSideMenus();
            jquery__WEBPACK_IMPORTED_MODULE_2___default()('#socrates-menu').effect('slide', { direction: 'left', mode: 'show' }, 1000);
            isSocratesMenuOpen = true;
            socrates(-1);
            jquery__WEBPACK_IMPORTED_MODULE_2___default()('#menu-satellite-collision').addClass('bmenu-item-selected');
            return;
        }
    }
};
var hideSideMenus = function () {
    jquery__WEBPACK_IMPORTED_MODULE_2___default()('#socrates-menu').effect('slide', { direction: 'left', mode: 'hide' }, 1000);
    jquery__WEBPACK_IMPORTED_MODULE_2___default()('#menu-satellite-collision').removeClass('bmenu-item-selected');
    isSocratesMenuOpen = false;
};
var onCruncherMessage = function () {
    if (socratesOnSatCruncher !== null) {
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.objectManager.setSelectedSat(socratesOnSatCruncher);
...
var findFutureDate = function (socratesObjTwo, row) {
    ...
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.propOffset = selectedDate.getTime() - today.getTime(); // Find the offset from today
    ...
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.propRealTime = Date.now(); // Reset realtime...this might not be necessary...
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.propTime();
}; // Allows passing -1 argument to socrates function to skip these steps
var socrates = function (row) {
    if (isNaN(row))
        throw new Error('SOCRATES: Row is not a number');
    // SOCRATES Variables
    var _a = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.socrates, socratesObjOne = _a.socratesObjOne, socratesObjTwo = _a.socratesObjTwo;
    ...
    if (row === -1 && socratesObjOne.length === 0 && socratesObjTwo.length === 0) {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_2___default().get('/SOCRATES.htm', function (socratesHTM) { return processSocratesHtm(socratesHTM); });
    }
    if (row !== -1) {
        // If an object was selected from the menu
        findFutureDate(socratesObjTwo, row); // Jump to the date/time of the collision
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.doSearch(socratesObjOne[row][1] + ',' + socratesObjTwo[row][0]); // Actually perform the search of the two objects
        socratesOnSatCruncher = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.getIdFromObjNum(socratesObjOne[row][1]);
    } // If a row was selected
};
var processSocratesHtm = function (socratesHTM) {
    var _a = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.socrates, socratesObjOne = _a.socratesObjOne, socratesObjTwo = _a.socratesObjTwo;
    // Load SOCRATES.htm so we can use it instead of index.htm
    var tableRowOne = jquery__WEBPACK_IMPORTED_MODULE_2___default()("[name='CATNR1']", socratesHTM).closest('tr'); // Find the row(s) containing the hidden input named CATNR1
    var tableRowTwo = jquery__WEBPACK_IMPORTED_MODULE_2___default()("[name='CATNR2']", socratesHTM).closest('tr'); // Find the row(s) containing the hidden input named CATNR2
    ...
    for (var i = 0; i < 20; i++) {
        ...
        tdT.appendChild(document.createTextNode(socratesDate[2] + ' ' + socratesDate[1] + ' ' + socratesDate[0] + ' - ' + _app_js_lib_helpers__WEBPACK_IMPORTED_MODULE_1__.stringPad.pad0(socratesTime[0], 2) + ':' + _app_js_lib_helpers__WEBPACK_IMPORTED_MODULE_1__.stringPad.pad0(socratesTime[1], 2) + ':' + _app_js_lib_helpers__WEBPACK_IMPORTED_MODULE_1__.stringPad.pad0(socratesTimeS[0], 2) + 'Z'));
        tdS1 = tr.insertCell();
        tdS1.appendChild(document.createTextNode(socratesObjOne[i][1]));
        tdS2 = tr.insertCell();
        tdS2.appendChild(document.createTextNode(socratesObjTwo[i][0]));
    }
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.socrates = { socratesObjOne: socratesObjOne, socratesObjTwo: socratesObjTwo };
};
...
//# sourceMappingURL=src_js_plugins_collisions_collisions_ts.857bd23b4a76b1c51d95.js.map