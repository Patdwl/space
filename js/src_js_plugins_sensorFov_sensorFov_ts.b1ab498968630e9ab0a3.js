...
const init = () => {
    ...
    _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.sensorFov = {
        enableFovView,
    };
};
const enableFovView = () => {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#menu-fov-bubble').addClass('bmenu-item-selected');
...
const bottomMenuClick = (iconName) => {
    const { sensorManager, satSet, uiManager } = _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs;
    if (iconName === 'menu-fov-bubble') {
        if (!sensorManager.checkSensorSelected()) {
            // No Sensor Selected
            if (settingsManager.plugins.topMenu)
                _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.adviceManager.adviceList.bubbleDisabled();
            uiManager.toast(`Select a Sensor First!`, 'caution');
            if (!jquery__WEBPACK_IMPORTED_MODULE_1___default()('#menu-fov-bubble:animated').length) {
                jquery__WEBPACK_IMPORTED_MODULE_1___default()('#menu-fov-bubble').effect('shake', {
                    distance: 10,
                });
            }
            return;
...
//# sourceMappingURL=src_js_plugins_sensorFov_sensorFov_ts.b1ab498968630e9ab0a3.js.map