...
_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satChange = {
    satChngTable: [],
};
var init = function () {
    // Add HTML
...
var uiManagerInit = function () {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#satChng-menu').resizable({
        handles: 'e',
        stop: function () {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#satChng-menu').on('click', '.satChng-object', function (evt) {
...
var satChng = function (row) {
    var satChngTable = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satChange.satChngTable;
    ...
    if (row === -1 && satChngTable.length === 0) {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_1___default().get('/analysis/satchng.json?v=' + _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.settingsManager.versionNumber).done(function (resp) {
            var _a;
            (_a = getSatChngJson(resp, satChngTable), resp = _a.resp, satChngTable = _a.satChngTable);
            _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satChange.satChngTable = satChngTable;
        });
    }
    if (row !== -1) {
        // If an object was selected from the menu
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.doSearch(satChngTable[row].SCC); // Actually perform the search of the two objects
        jquery__WEBPACK_IMPORTED_MODULE_1___default()('#anal-sat').val(satChngTable[row].SCC);
    } // If a row was selected
};
var hideSideMenus = function () {
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#satChng-menu').effect('slide', { direction: 'left', mode: 'hide' }, 1000);
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#menu-satChng').removeClass('bmenu-item-selected');
...
var bottomMenuClick = function (iconName) {
    if (iconName === 'menu-satChng') {
        if (issatChngMenuOpen) {
            ...
            _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.hideSideMenus();
            jquery__WEBPACK_IMPORTED_MODULE_1___default()('#satChng-menu').effect('slide', { direction: 'left', mode: 'show' }, 1000);
            issatChngMenuOpen = true;
            satChng(-1);
            jquery__WEBPACK_IMPORTED_MODULE_1___default()('#menu-satChng').addClass('bmenu-item-selected');
            return;
        }
    }
};
var getSatChngJson = function (resp, satChngTable) {
    resp = __spreadArray([], __read(new Set(resp)), false);
    for (var i = 0; i < resp.length; i++) {
        var prefix = resp[i].year > 50 ? '19' : '20';
        var year = parseInt(prefix + resp[i].year.toString());
        var date = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.dateFromDay(year, resp[i].day);
    ...
    for (var i = 0; i < Math.min(satChngTable.length, 20); i++) {
        ...
        var sat = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.getSat(_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.getIdFromObjNum(satChngTable[i].SCC));
    ...
    return { resp: resp, satChngTable: satChngTable };
};
...
//# sourceMappingURL=src_js_plugins_satChanges_satChanges_ts.dfa3b4a4e4863a363d78.js.map