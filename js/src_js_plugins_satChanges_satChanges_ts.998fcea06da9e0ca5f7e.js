...
/* harmony import */ var _app_js_uiManager_uiManager__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @app/js/uiManager/uiManager */ "./src/js/uiManager/uiManager.ts");
...
const uiManagerInit = () => {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_3___default()('#bottom-icons').append(_app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.html `
      ...
      `);
    jquery__WEBPACK_IMPORTED_MODULE_3___default()('#satChng-menu').resizable({
        handles: 'e',
        stop: function () {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_3___default()('#satChng-menu').on('click', '.satChng-object', function (evt) {
...
const satChng = (row) => {
    const { satChange } = _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs;
    ...
    if (row === -1 && (satChngTable === null || satChngTable === void 0 ? void 0 : satChngTable.length) === 0) {
        ...
        jquery__WEBPACK_IMPORTED_MODULE_3___default().get('/analysis/satchng.json?v=' + settingsManager.versionNumber).done((resp) => {
    ...
    if (row !== -1) {
        ...
        if (!satChngTable[row].SCC)
            return;
        _app_js_uiManager_uiManager__WEBPACK_IMPORTED_MODULE_2__.uiManager.doSearch(satChngTable[row].SCC.toString()); // Actually perform the search of the two objects
        jquery__WEBPACK_IMPORTED_MODULE_3___default()('#anal-sat').val(satChngTable[row].SCC.toString());
    } // If a row was selected
};
const hideSideMenus = () => {
    jquery__WEBPACK_IMPORTED_MODULE_3___default()('#satChng-menu').effect('slide', { direction: 'left', mode: 'hide' }, 1000);
    jquery__WEBPACK_IMPORTED_MODULE_3___default()('#menu-satChng').removeClass('bmenu-item-selected');
...
const bottomMenuClick = (iconName) => {
    if (iconName === 'menu-satChng') {
        ...
        if (issatChngMenuOpen) {
            ...
            uiManager.hideSideMenus();
            jquery__WEBPACK_IMPORTED_MODULE_3___default()('#satChng-menu').effect('slide', { direction: 'left', mode: 'show' }, 1000);
            issatChngMenuOpen = true;
            satChng(-1);
            jquery__WEBPACK_IMPORTED_MODULE_3___default()('#menu-satChng').addClass('bmenu-item-selected');
            return;
        }
    }
};
const getSatChngJson = (resp, satChngTable) => {
    resp = [...new Set(resp)];
...
//# sourceMappingURL=src_js_plugins_satChanges_satChanges_ts.998fcea06da9e0ca5f7e.js.map