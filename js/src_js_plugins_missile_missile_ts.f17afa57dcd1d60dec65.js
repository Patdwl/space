...
_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.missileManager = _missileManager__WEBPACK_IMPORTED_MODULE_2__.missileManager;
...
var updateLoop = function () {
    if (typeof _missileManager__WEBPACK_IMPORTED_MODULE_2__.missileManager != 'undefined' && _missileManager__WEBPACK_IMPORTED_MODULE_2__.missileManager.missileArray.length > 0) {
        var _a = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs, drawManager = _a.drawManager, orbitManager = _a.orbitManager;
...
var missileSubmit = function () {
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#loading-screen').fadeIn(1000, function () {
        var _a = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs, uiManager = _a.uiManager, satSet = _a.satSet, timeManager = _a.timeManager;
        jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-error').hide();
        var type = parseFloat(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-type').val());
        var attacker = parseFloat(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-attacker').val());
        var lauLat = parseFloat(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-lat-lau').val());
        var lauLon = parseFloat(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-lon-lau').val());
        var target = parseFloat(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-target').val());
        var tgtLat = parseFloat(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-lat').val());
        var tgtLon = parseFloat(jquery__WEBPACK_IMPORTED_MODULE_1___default()('#ms-lon').val());
        // var result = false;
        ...
        if (type !== 0) {
            uiManager.toast("".concat(sim, " Loaded"), 'standby', true);
        }
        if (type === 0) {
            if (target === -1) {
                // Custom Target
...
var uiManagerInit = function () {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#missile').on('submit', function (e) {
        missileSubmit();
        e.preventDefault();
    });
...
/* harmony import */ var _app_js_timeManager_transforms__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @app/js/timeManager/transforms */ "./src/js/timeManager/transforms.ts");
...
var _a = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs, satellite = _a.satellite, sensorManager = _a.sensorManager, timeManager = _a.timeManager;
var EarthRadius, EarthMass, FuelDensity, BurnRate, WarheadMass, R, G, h;
var missileArray = [];
// External Functions
// This function stalls Jest for multiple minutes.
/* istanbul ignore next */
var MassRaidPre = function (time, simFile) {
    var _a = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs, satSet = _a.satSet, orbitManager = _a.orbitManager;
    missileManager.clearMissiles();
    jquery__WEBPACK_IMPORTED_MODULE_3___default().get(simFile, function (missileArray) {
        ...
        for (var i = 0; i < missileArray.length; i++) {
            ...
            if (MissileObject) {
                ...
                _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.satCruncher.postMessage({
                    id: MissileObject.id,
                    typ: 'newMissile',
        ...
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.doSearch('RV_');
    });
};
var clearMissiles = function () {
    ...
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.doSearch('');
...
// This function stalls Jest for multiple minutes.
/* istanbul ignore next */
var Missile = function (CurrentLatitude, CurrentLongitude, TargetLatitude, TargetLongitude, NumberWarheads, MissileObjectNum, CurrentTime, MissileDesc, Length, Diameter, NewBurnRate, MaxMissileRange, country, minAltitude) {
    ...
    var LatList = [];
    var LongList = [];
    var _b = __read(_CoordinateCalculator(CurrentLatitude, CurrentLongitude, TargetLatitude, TargetLongitude), 6), EstLatList = _b[0], EstLongList = _b[1], ArcLength = _b[3], EstDistanceList = _b[4], GoalDistance = _b[5];
    if (ArcLength < 320000) {
    ...
    if (ArcLength > MaxMissileRange * 1000) {
        ...
        // console.debug('Please choose different target coordinates.');
        missileManager.lastMissileErrorType = 'critical';
        missileManager.lastMissileError = "Error: This missile has a maximum distance of ".concat(MaxMissileRange, " km.");
        return 0;
    }
    ...
    while (FuelMass / FuelDensity / FuelVolume > 0.4 && Altitude >= 0) {
        ...
        for (var i = 0; i < EstDistanceList.length; i++) {
    ...
    while (FuelMass / FuelDensity / FuelVolume > 0.19 && Altitude >= 0) {
        var iterationFunOutput = _IterationFun(FuelArea1, FuelMass, RocketArea, Altitude, RocketCasingMass2, NozzleAltitude2, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient);
        FuelMass = iterationFunOutput[0];
        RocketMass = iterationFunOutput[1];
        Tatm = iterationFunOutput[2];
        Patm = iterationFunOutput[3];
    ...
    while (FuelMass / FuelDensity / FuelVolume > 0 && Altitude >= 0) {
        var iterationFunOutput = _IterationFun(FuelArea2, FuelMass, RocketArea, Altitude, RocketCasingMass3, NozzleAltitude3, drdt, dthetadt, Distance, ArcDistance, MassIn, AngleCoefficient);
        FuelMass = iterationFunOutput[0];
        RocketMass = iterationFunOutput[1];
        Tatm = iterationFunOutput[2];
        Patm = iterationFunOutput[3];
...
var getMissileTEARR = function (missile, sensor) {
    ...
    var j = (0,_app_js_timeManager_transforms__WEBPACK_IMPORTED_MODULE_2__.jday)(now.getUTCFullYear(), now.getUTCMonth() + 1, // NOTE:, this function requires months in range 1-12.
    now.getUTCDate(), now.getUTCHours(), now.getUTCMinutes(), now.getUTCSeconds()); // Converts time to jday (TLEs use epoch year/day)
    j += now.getUTCMilliseconds() * _app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.MILLISECONDS_PER_DAY;
    ...
    var cosLat = Math.cos(missile.latList[curMissileTime] * _app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.DEG2RAD);
    var sinLat = Math.sin(missile.latList[curMissileTime] * _app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.DEG2RAD);
    var cosLon = Math.cos(missile.lonList[curMissileTime] * _app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.DEG2RAD + gmst);
    var sinLon = Math.sin(missile.lonList[curMissileTime] * _app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.DEG2RAD + gmst);
    var x = (_app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.RADIUS_OF_EARTH + missile.altList[curMissileTime]) * cosLat * cosLon;
    var y = (_app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.RADIUS_OF_EARTH + missile.altList[curMissileTime]) * cosLat * sinLon;
    var z = (_app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.RADIUS_OF_EARTH + missile.altList[curMissileTime]) * sinLat;
    ...
    try {
        ...
        currentTEARR.az = lookAngles.az * _app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.RAD2DEG;
        currentTEARR.el = lookAngles.el * _app_js_lib_constants_js__WEBPACK_IMPORTED_MODULE_1__.RAD2DEG;
...
var _CoordinateCalculator = function (CurrentLatitude, CurrentLongitude, TargetLatitude, TargetLongitude) {
    ...
    for (var i = 0; i < LatList1.length; i++)
        EstLatList.push(LatList1[i]);
    for (var i = 0; i < LatList2.length; i++)
        EstLatList.push(LatList2[i]);
    for (var i = 0; i < LatList3.length; i++)
        EstLatList.push(LatList3[i]);
    for (var i = 0; i < LongList1.length; i++)
        EstLongList.push(LongList1[i]);
    for (var i = 0; i < LongList2.length; i++)
        EstLongList.push(LongList2[i]);
    for (var i = 0; i < LongList3.length; i++)
...
//# sourceMappingURL=src_js_plugins_missile_missile_ts.f17afa57dcd1d60dec65.js.map