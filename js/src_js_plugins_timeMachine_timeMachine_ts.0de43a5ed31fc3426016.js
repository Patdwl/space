...
const uiManagerInit = () => {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#bottom-icons').append(_app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.html `
      ...
      `);
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#time-machine-icon').on('click', timeMachineIconClick);
};
const bottomMenuClick = (iconName) => {
    const { orbitManager, groupsManager, satSet, colorSchemeManager } = _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs;
    if (iconName === 'menu-time-machine') {
        if (orbitManager.isTimeMachineRunning) {
            ...
            jquery__WEBPACK_IMPORTED_MODULE_1___default()('#menu-time-machine').removeClass('bmenu-item-selected');
            return;
        }
        else {
            // Merge to one variable?
            ...
            jquery__WEBPACK_IMPORTED_MODULE_1___default()('#menu-time-machine').addClass('bmenu-item-selected');
...
const orbitManagerInit = () => {
    const { orbitManager, satSet, colorSchemeManager, groupsManager } = _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs;
    orbitManager.playNextSatellite = (runCount, year) => {
        if (!_app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.orbitManager.isTimeMachineVisible)
            return;
        ...
        if (year >= 59 && year < 100) {
            _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.toast(`Time Machine In Year 19${year}!`, 'normal');
        }
        else {
            const yearStr = year < 10 ? `0${year}` : `${year}`;
            _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.toast(`Time Machine In Year 20${yearStr}!`, 'normal');
        }
...
const timeMachineIconClick = () => {
    const { searchBox } = _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs;
    if (jquery__WEBPACK_IMPORTED_MODULE_1___default()('#time-machine-menu').css('display') === 'block') {
        jquery__WEBPACK_IMPORTED_MODULE_1___default()('#time-machine-menu').hide();
    }
    else {
        jquery__WEBPACK_IMPORTED_MODULE_1___default()('#time-machine-menu').show();
...
//# sourceMappingURL=src_js_plugins_timeMachine_timeMachine_ts.0de43a5ed31fc3426016.js.map