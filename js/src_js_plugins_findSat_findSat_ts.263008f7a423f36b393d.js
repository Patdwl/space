...
/* harmony export */   "searchAzElRange": () => (/* binding */ searchAzElRange),
...
var checkPeriod = function (possibles, minPeriod, maxPeriod) {
    ...
    if (periodRes.length >= 200) {
        jquery__WEBPACK_IMPORTED_MODULE_2___default()('#findByLooks-results').text('Limited to 200 Results!');
    }
    return periodRes;
};
...
var searchAzElRange = function (azimuth, elevation, range, inclination, azMarg, elMarg, rangeMarg, incMarg, period, periodMarg, rcs, rcsMarg, objtypeStr) {
    ...
    if (!isCheckEl && !isCheckRange && !isCheckAz && !isCheckInclination && !isCheckPeriod && !isCheckRcs)
        []; // Ensure there is a number typed.
    if (!isCheckAzMarg) {
        azMarg = 5;
    }
    if (!isCheckElMarg) {
    ...
    var res = [];
    var s = 0;
    var satData = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.satData;
    for (var i = 0; i < satData.length; i++) {
        if (satData[i].static || satData[i].missile || !satData[i].active) {
            continue;
        }
        res.push(satData[i]);
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satellite.getTEARR(res[s]);
        res[s].az = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satellite.currentTEARR.az;
        res[s].el = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satellite.currentTEARR.el;
        res[s].rng = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satellite.currentTEARR.rng;
        res[s].inView = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satellite.currentTEARR.inView;
        s++;
    }
    ...
    for (var i = 0; i < res.length; i++) {
        ...
        if (i < res.length - 1) {
            jquery__WEBPACK_IMPORTED_MODULE_2___default()('#search').val(jquery__WEBPACK_IMPORTED_MODULE_2___default()('#search').val() + res[i].SCC_NUM + ',');
        }
        else {
            jquery__WEBPACK_IMPORTED_MODULE_2___default()('#search').val(jquery__WEBPACK_IMPORTED_MODULE_2___default()('#search').val() + res[i].SCC_NUM);
        }
    ...
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.doSearch(jquery__WEBPACK_IMPORTED_MODULE_2___default()('#search').val());
    // console.log(sccList);
    return res;
};
...
var checkObjtype = function (possibles, objtype) {
    var objtypeRes = [];
    for (var i = 0; i < possibles.length; i++) {
...
var newLaunchSubmit = function () {
    var scc = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#nl-scc').val();
    var satId = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.getIdFromObjNum(scc);
    var sat = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.getSat(satId);
    var upOrDown = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#nl-updown').val();
    var launchFac = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#nl-facility').val();
    var launchLat, launchLon;
    if (_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.objectManager.isLaunchSiteManagerLoaded) {
        for (var launchSite in _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.objectManager.launchSiteManager.launchSiteList) {
            if (_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.objectManager.launchSiteManager.launchSiteList[launchSite].name === launchFac) {
                launchLat = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.objectManager.launchSiteManager.launchSiteList[launchSite].lat;
                launchLon = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.objectManager.launchSiteManager.launchSiteList[launchSite].lon;
            }
        }
    }
    if (launchLon > 180) {
    ...
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.propOffset = quadZTime.getTime() - today.getTime(); // Find the offset from today
    ...
    _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.satCruncher.postMessage({
        ...
        typ: 'offset',
        dat: _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.propOffset.toString() + ' ' + (1.0).toString(),
    });
    var TLEs = _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satellite.getOrbitByLatLon(sat, launchLat, launchLon, upOrDown, _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.propOffset);
    var TLE1 = TLEs[0];
    var TLE2 = TLEs[1];
    if (_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satellite.altitudeCheck(TLE1, TLE2, _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.timeManager.propOffset) > 1) {
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.satSet.satCruncher.postMessage({
            typ: 'satEdit',
            id: satId,
            active: true,
...
var bottomMenuClick = function (iconName) {
    if (iconName === 'menu-find-sat') {
        if (isFindByLooksMenuOpen) {
            ...
            _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.hideSideMenus();
            return;
        }
        else {
            if (_app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.settingsManager.isMobileModeEnabled)
                _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.searchToggle(false);
            _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.hideSideMenus();
            jquery__WEBPACK_IMPORTED_MODULE_2___default()('#findByLooks-menu').effect('slide', { direction: 'left', mode: 'show' }, 1000);
            isFindByLooksMenuOpen = true;
            jquery__WEBPACK_IMPORTED_MODULE_2___default()('#menu-find-sat').addClass('bmenu-item-selected');
            return;
        }
    }
};
var hideSideMenus = function () {
    jquery__WEBPACK_IMPORTED_MODULE_2___default()('#findByLooks-menu').effect('slide', { direction: 'left', mode: 'hide' }, 1000);
    jquery__WEBPACK_IMPORTED_MODULE_2___default()('#menu-find-sat').removeClass('bmenu-item-selected');
    isFindByLooksMenuOpen = false;
};
var init = function () {
    // Add HTML
...
var findByLooksSubmit = function () {
    var fblAzimuth = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-azimuth').val();
    var fblElevation = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-elevation').val();
    var fblRange = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-range').val();
    var fblInc = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-inc').val();
    var fblPeriod = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-period').val();
    var fblRcs = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-rcs').val();
    var fblAzimuthM = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-azimuth-margin').val();
    var fblElevationM = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-elevation-margin').val();
    var fblRangeM = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-range-margin').val();
    var fblIncM = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-inc-margin').val();
    var fblPeriodM = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-period-margin').val();
    var fblRcsM = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-rcs-margin').val();
    var fblType = jquery__WEBPACK_IMPORTED_MODULE_2___default()('#fbl-type').val();
    jquery__WEBPACK_IMPORTED_MODULE_2___default()('#search').val(''); // Reset the search first
    ...
    if (typeof res === 'undefined') {
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.toast("No Search Criteria Entered", 'critical');
    }
    else if (res.length === 0) {
        _app_js_api_externalApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.uiManager.toast("No Satellites Found", 'critical');
    }
};
...
//# sourceMappingURL=src_js_plugins_findSat_findSat_ts.263008f7a423f36b393d.js.map