...
const init = () => {
    ...
    let infoOverlayDOM = [];
    uiManager.updateNextPassOverlay = (nextPassArray, isForceUpdate) => {
        ...
        if ((timeManager.realTime > _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs.watchlist.lastOverlayUpdateTime * 1 + 10000 && objectManager.selectedSat === -1 && !mainCamera.isDragging && mainCamera.zoomLevel === mainCamera.zoomTarget) || isForceUpdate) {
...
const updateWatchlist = (updateWatchlistList, updateWatchlistInViewList) => {
...
`);
    ...
    jquery__WEBPACK_IMPORTED_MODULE_3___default()('#watchlist-list').on('click', '.watchlist-remove', function () {
        const satId = jquery__WEBPACK_IMPORTED_MODULE_3___default()(this).data('sat-id');
...
const pushOverlayElement = (satSet, nextPassArray, s, propTime, infoOverlayDOM) => {
    ...
    if (nextPassArray[s].time - propTime < -1000 * 60 * 5 && !satInView)
        return;
    // Get the pass Time
    const time = (0,_app_js_lib_external_dateFormat_js__WEBPACK_IMPORTED_MODULE_1__.dateFormat)(nextPassArray[s].time, 'isoTime', true);
...
const watchlistContentEvent = (e, satId) => {
    const { satSet, sensorManager } = _app_js_api_keepTrackApi__WEBPACK_IMPORTED_MODULE_0__.keepTrackApi.programs;
    satId !== null && satId !== void 0 ? satId : (satId = satSet.getIdFromObjNum(jquery__WEBPACK_IMPORTED_MODULE_3___default()('#watchlist-new').val()));
    let duplicate = false;
    for (let i = 0; i < watchlistList.length; i++) {
        // No duplicates
        if (watchlistList[i] === satId)
...
//# sourceMappingURL=src_js_plugins_watchlist_watchlist_ts.07abaef4a8dccdcc4c5c.js.map