...
import { SceneComponent } from '../three-manager';
import { earthShaders } from './earth/earth-shaders';

// export class Earth3_ {
//   private attribs_ = {
//     aVertexPosition: 0,
//     aVertexNormal: 0,
//     aTexCoord: 0,
...
//   draw(pMatrix: mat4, tgtBuffer: WebGLFramebuffer, altNightTexBind?: (gl: WebGL2RenderingContext, textureNight: WebGLTexture, textureDay: WebGLTexture) => void) {
...
//       this.onImageLoaded_();
//     };
//     img.src = Earth3.getSrcBump_(this.settings_);
...
//       this.onImageLoaded_();
//     };
//     img.src = Earth3.getSrcDay_(this.settings_);
//   }
...
//       this.onImageLoaded_();
//     };
//     img.src = Earth3.getSrcNight_(this.settings_);
//   }
...
//       this.onImageLoaded_();
//     };
//     img.src = Earth3.getSrcSpec_(this.settings_);
...
//   private async loadHiResNight_(): Promise<void> {
//     try {
//       const img = new Image();
//       img.onload = () => {
//         if (!this.settings_.isBlackEarth) {
...
//         this.onImageLoaded_();
//       };
//       img.src = Earth3.getSrcHiResNight_(this.settings_);
//     } catch (e) {
//       console.debug(e);
//     }
//   }

//   private async loadHiRes_(): Promise<void> {
//     try {
//       const img = new Image();
//       img.onload = () => {
//         if (!this.settings_.isBlackEarth) {
...
//         this.onImageLoaded_();
//       };
//       img.src = Earth3.getSrcHiResDay_(this.settings_);
...
export class Earth3 implements SceneComponent {
  ...
  init(scene: THREE.Scene) {
    this.dayTexture = new THREE.TextureLoader().load(Earth3.getSrcDay_(settingsManager));
    this.nightTexture = new THREE.TextureLoader().load(Earth3.getSrcNight_(settingsManager));
    this.bumpTexture = new THREE.TextureLoader().load(Earth3.getSrcBump_(settingsManager));
    this.earthSpecularMap = new THREE.TextureLoader().load(Earth3.getSrcSpec_(settingsManager));

    const dayMaterial = new THREE.RawShaderMaterial({
      uniforms: {
        uZoomModifier: {
          value:
            keepTrackApi.getMainCamera().cameraType === CameraType.FIXED_TO_SAT ||
        ...
        uGlow: { value: keepTrackApi.getDrawManager().sceneManager.earth.glowNumber },
        uCamPos: { value: keepTrackApi.getMainCamera().getForwardVector() },
        uNormalMatrix: { value: keepTrackApi.getDrawManager().sceneManager.earth['nMatrix_'] },
        uMvMatrix: { value: keepTrackApi.getDrawManager().sceneManager.earth['mvMatrix_'] },
        uPMatrix: { value: keepTrackApi.getDrawManager().pMatrix },
        uCamMatrix: { value: keepTrackApi.getMainCamera().camMatrix },
        uLightDirection: { value: keepTrackApi.getDrawManager().sceneManager.earth.lightDirection },
        uAmbientLightColor: { value: new THREE.Vector3(0.1, 0.1, 0.1) },
        uDirectionalLightColor: { value: new THREE.Vector3(1.0, 1.0, 1.0) },
        uIsDrawAtmosphere: { value: settingsManager.isDrawAtmosphere ? 1.0 : 0.0 },
        uIsDrawAurora: { value: settingsManager.isDrawAurora ? 1.0 : 0.0 },
        uSampler: { value: this.dayTexture },
        uNightSampler: { value: this.nightTexture },
        uBumpMap: { value: this.bumpTexture },
        uSpecMap: { value: this.earthSpecularMap },
      },
      glslVersion: THREE.GLSL3,
      vertexShader: earthShaders.vert,
      fragmentShader: earthShaders.frag,
    });

    const geometry = new THREE.SphereGeometry(2, settingsManager.earthNumLatSegs, settingsManager.earthNumLonSegs);
    this.mesh = new THREE.Mesh(geometry, dayMaterial);
  ...
  update(_scene?: THREE.Scene): void | Promise<void> {
    if (this.mesh) {
      this.mesh.rotation.y += 0.005;

      (this.mesh.material as THREE.ShaderMaterial).uniforms.uZoomModifier.value = keepTrackApi.getMainCamera().zoomLevel();
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uGlow.value = keepTrackApi.getDrawManager().sceneManager.earth.glowNumber;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uCamPos.value = keepTrackApi.getMainCamera().getForwardVector();
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uNormalMatrix.value = keepTrackApi.getDrawManager().sceneManager.earth['nMatrix_'];
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uMvMatrix.value = keepTrackApi.getDrawManager().sceneManager.earth['mvMatrix_'];
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uPMatrix.value = keepTrackApi.getDrawManager().pMatrix;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uCamMatrix.value = keepTrackApi.getMainCamera().camMatrix;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uLightDirection.value = keepTrackApi.getDrawManager().sceneManager.earth.lightDirection;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uAmbientLightColor.value = new THREE.Vector3(0.1, 0.1, 0.1);
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uDirectionalLightColor.value = new THREE.Vector3(1.0, 1.0, 1.0);
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uIsDrawAtmosphere.value = settingsManager.isDrawAtmosphere ? 1.0 : 0.0;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uIsDrawAurora.value = settingsManager.isDrawAurora ? 1.0 : 0.0;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uSampler.value = this.dayTexture;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uNightSampler.value = this.nightTexture;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uBumpMap.value = this.bumpTexture;
      (this.mesh.material as THREE.ShaderMaterial).uniforms.uSpecMap.value = this.earthSpecularMap;
    }
  }
   ...
   */
  static getSrcHiResDay_(settings: SettingsManager): string {
    if (!settings.installDirectory) throw new Error('settings.installDirectory is undefined');
   ...
   */
  static getSrcHiResNight_(settings: SettingsManager): string {
    if (!settings.installDirectory) throw new Error('settings.installDirectory is undefined');
